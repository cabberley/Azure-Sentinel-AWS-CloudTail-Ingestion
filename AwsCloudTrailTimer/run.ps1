<#  
    Title:          Azure Sentinel CloudTrail Ingestion - Time Trigger to queue list of AWS SQS Queues to check
    Language:       PowerShell
    Version:        0.1.1
...
$SQSQueuesTable = $env:SQSConfig
$AzureSQSQueueName = $env:SQSCloudTrailQueueName

#####If Environment did not supply set the default names
if ($null -eq $SQSQueuesTable) { $SQSQueuesTable = 'cloudtrailconfigs' }
...
Foreach ($AWSQueue in $SQSQueuesConfigs){
    $AzureQmessage = `
    $AWSQueue.sqsQueueName + ';' `
    + $AWSQueue.AWSRegion + ';' `
    + $AWSQueue.AWSProfileSQS + ';' `
    + $AWSQueue.AWSProfileS3
